...
bola_raio = 0.5 # metros
# Criando a esfera com arrasto (cor azul)
bola_com_arrasto = sphere(pos=vector(-1, 0.59, 0), radius=0.5, color=color.blue, make_trail=True)

# Criando a esfera sem arrasto (cor vermelha)
bola_sem_arrasto = sphere(pos=vector(1, 0.59, 0), radius=0.5, color=color.red, make_trail=True)

# Parâmetros do sistema
m = 1.0  # Massa do objeto (kg)
g = 9.8  # Aceleração gravitacional (m/s^2)
p = 1.225 # 1.225 kilogram per cubic meter (kg/m3)
coeficiente_arrasto = 0.47  # Coeficiente de arrasto (kg/s)
...
area = 3.14 * bola_raio**2  # Area tranversal
...
while bola_com_arrasto.pos.y > bola_com_arrasto.radius or bola_sem_arrasto.pos.y > bola_sem_arrasto.radius:
    ...
    if bola_com_arrasto.pos.y > bola_com_arrasto.radius:
        # Para a bolinha com arrasto
        if bola_com_arrasto.pos.y > bola_com_arrasto.radius:
        # Verifica a direção do movimento para ajustar o sinal da força de arrasto
            if v_com_arrasto.y > 0:
                f_arrasto_valor = vector(0, -(1/2) * coeficiente_arrasto * p * area * (v_com_arrasto.y ** 2), 0)
            else:
                f_arrasto_valor = vector(0, (1/2) * coeficiente_arrasto * p * area * (v_com_arrasto.y ** 2), 0)
